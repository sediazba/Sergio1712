...
int main(void)
{
    ...
    s[0] = 2.0; // y
    s[1] = -1.0; // dydt
    euler(s, 0.0001, 5.0, 0.1); // al t iniciar en 0 y calcular las componentes de la derivada el valor no existe y se detien, por ello pongo un valor cerca a 0

    s[0] = 2.0; // y
    s[1] = -1.0; // dydt
    rk4(s, 0.0001, 5.0, 0.1);
...
void euler(std::vector <double> & s, double t0, double tf, double h)
{
    ...
    for(double t = t0; t <= tf; t = t + h){
        f(s, dsdt, t);
        fout << t << "\t" << s[0] << "\t" << s[1] << dsdt[0] << "t" << dsdt[1] << "\n";
