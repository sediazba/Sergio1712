...
int main(int argc, char *argv[]) {
  ...
  PARAMS["K"] = 200.4567; // Elastic constant, N/m
  PARAMS["B"] = 0.0; // Damping strength, 1/s
  PARAMS["Prx"] = 5.12; // Pared derecha
  PARAMS["Plx"] = 0.0; // Pared izquierda
  PARAMS["Pz"] = 0.5; // Pared superior
  PARAMS["DT"] = 0.001; // Time step size, s
  PARAMS["T0"] = 0.0; // Initial time, s
  PARAMS["TF"] = 5.3456; // Final time, s 
...
void compute_forces(std::vector<Particle> &particles, std::map<std::string, double> &params) {
  ...
  for (auto & body : particles) {
    double delta = body.rad + body.R[0] - params["Prx"];
    if (delta >= 0) {
      body.F[0] -= params["K"]*delta;
    }
  }
  for (auto & body : particles) {
    double delta = body.rad - body.R[0] + params["Plx"];
    if (delta >= 0) {
      body.F[0] += params["K"]*delta;
    }
  }
...
void print_gnuplot(const std::vector<Particle> &particles,
  ...
  for (const auto & body : particles) {
    std::cout << body.mass << " " << body.R[0] << " " << body.R[2] << " " << body.V[0] << " " << body.V[2] << " " << body.F[0] << " " << body.F[2] << " ";
  }
  std::cout << "\n";
}
